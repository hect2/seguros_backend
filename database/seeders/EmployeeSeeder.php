...
class EmployeeSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($i = 0; $i < 20; $i++) {
            self::processCreateEmployee($statuses, $users->random()->id, $offices->random(), $positionTypes);
        }

        $office = Office::where("code", 'OF009')->first();
        for ($i = 0; $i < 4; $i++) {
            self::processCreateEmployee($statuses, $users->random()->id, $office, $positionTypes);
        }

        $office = Office::where("code", 'OF008')->first();
        for ($i = 0; $i < 4; $i++) {
            self::processCreateEmployee($statuses, $users->random()->id, $office, $positionTypes);
        }

        $office = Office::where("code", 'OF004')->first();
        for ($i = 0; $i < 4; $i++) {
            self::processCreateEmployee($statuses, $users->random()->id, $office, $positionTypes);
        }

        $office = Office::where("code", 'OF012')->first();
        for ($i = 0; $i < 4; $i++) {
            self::processCreateEmployee($statuses, $users->random()->id, $office, $positionTypes);
        }
    }

    public function processCreateEmployee($statuses, $user_id, $office, $positionTypes): void
    {
        ...
        Tracking::create([
            ...
            'responsible' => fake()->boolean() ? $user_id : null,
            'approval_date' => null,
        ...
        $user_responsible_id = fake()->boolean() ? $user_id : null;
